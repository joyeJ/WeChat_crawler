...
try:
    conn = MySQLdb.connect(host='localhost', user='root', passwd='123', db='MesPush', port=3306)
    cur = conn.cursor()
...
class weixin_spider:
    ...
    def get_selenium_js_html(self, url):
        ...
        time.sleep(3)
        # 执行js得到整个页面内容
        # document.documentElement 是 html 的根节点
...
#    gongzhonghao = raw_input(u'输入要爬取的公众号')
    try:
        cur.execute('select WeChat_knowledge from WeChat_name')
    ...
    for i in range(numrows):
        row = cur.fetchone()
        gongzhonghao = row[0]
        if gongzhonghao == None :
            gongzhonghao = 'xgz_1009'
        weixin_spider(gongzhonghao).run()
        time.sleep(3)

    try:
        cur.execute('select WeChat_fun from WeChat_name')
    ...
    try:
        cur.execute('select WeChat_news from WeChat_name')
    ...
    try:
        cur.execute('select WeChat_health from WeChat_name')
